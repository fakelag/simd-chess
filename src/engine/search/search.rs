...
impl<'a> Search<'a> {
    ...
    fn go(&mut self, alpha: Eval, beta: Eval, depth: u8) -> Eval {
        ...
        let see_prune_data = if apply_pruning
            && !in_check
            && depth > 1
            && depth <= SEE_QUIET_PRUNE_MAX_DEPTH.max(SEE_CAPTURE_PRUNE_MAX_DEPTH)
        {
            ...
            Some((bb_black, bb_white, bb_pieces, pins))
        } else {
            None
        };
        ...
        for mv_index in 0..move_count {
            ...
            // SEE-based pruning (before make_move — avoids the make/unmake cost).
            // Guards: non-PV node, not in check, at least one legal move already found.
            if apply_pruning && !in_check && num_legal_moves > 0 && depth > 1 {
